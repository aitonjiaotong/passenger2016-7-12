...
public class ApiConnector {
	...
	private void get(String request, VolleyListener listener) {
		HTTPUtils.get(mContext, request, listener);
	}

	public void getInitData(int animeCount, int featureCount,
			int advertiseCount, VolleyListener listener) {
		...
		String request = String.format(INIT_REQUEST_URL, API_KEY, timeStamp,
				animeCount, featureCount, advertiseCount, access_token);
		get(request, listener);
	}

	public void getList(int page, VolleyListener listener) {
		long timeStamp = System.currentTimeMillis() / 1000L;
		TreeMap<String, String> params = new TreeMap<String, String>();
		params.put("api_key", API_KEY);
		params.put("timestamp", String.valueOf(timeStamp));
		...
		String access_token = ApiUtils.getAccessToken(params, API_SECRET);
		String request = String.format(ANIMATION_REQUEST_URL, API_KEY,
				timeStamp, page, access_token);
		get(request, listener);
	}

	public void getRandom(int count, VolleyListener listener) {
		long timeStamp = System.currentTimeMillis() / 1000L;
		TreeMap<String, String> params = new TreeMap<String, String>();
		params.put("api_key", API_KEY);
		params.put("timestamp", String.valueOf(timeStamp));
		...
		String access_token = ApiUtils.getAccessToken(params, API_SECRET);
		String request = String.format(ANIMATION_RANDOM_URL, API_KEY,
				timeStamp, count, access_token);
		get(request, listener);
	}

	public void getDetail(int vid, VolleyListener listener) {
		long timestamp = System.currentTimeMillis() / 1000L;
		TreeMap<String, String> params = new TreeMap<String, String>();
		params.put("api_key", API_KEY);
		params.put("timestamp", String.valueOf(timestamp));
		...
		String access_token = ApiUtils.getAccessToken(params, API_SECRET);
		String request = String.format(ANIMATION_DETAIL_URL, API_KEY,
				timestamp, vid, access_token);
		get(request, listener);
	}

	public void getCategory(int categoryId, int page, int count,
		...
		params.put("limit", String.valueOf(count));
		String access_token = ApiUtils.getAccessToken(params, API_SECRET);
		String request = String.format(CATEGORY_REQUEST_URL, API_KEY,
				timeStamp, page, categoryId, count, access_token);
		get(request, listener);
	}
	...
	public void getALLRecommend(int count, VolleyListener listener) {
		long timestamp = System.currentTimeMillis() / 1000L;
		TreeMap<String, String> params = new TreeMap<String, String>();
		params.put("api_key", API_KEY);
		params.put("timestamp", String.valueOf(timestamp));
		...
		String access_token = ApiUtils.getAccessToken(params, API_SECRET);
		String request = String.format(RECOMMEND_ALL_REQUEST, API_KEY,
				timestamp, count, access_token);
		get(request, listener);
	}

	public void getCategoryRecommend(int categoryId, int count,
			VolleyListener listener) {
		long timestamp = System.currentTimeMillis() / 1000L;
		TreeMap<String, String> params = new TreeMap<String, String>();
		params.put("api_key", API_KEY);
		...
		params.put("category", String.valueOf(categoryId));
		String access_token = ApiUtils.getAccessToken(params, API_SECRET);
		String request = String.format(RECOMMEND_CATEGORY_REQUEST, API_KEY,
				timestamp, categoryId, count, access_token);
		get(request, listener);
	}
}
